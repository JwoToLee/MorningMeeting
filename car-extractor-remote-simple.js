// ==UserScript==
// @name         HAESL CAR Batch Extractor - Remote Controlled
// @namespace    http://tampermonkey.net/
// @version      2.0
// @description  Remote controlled CAR Batch Extractor with GitHub access management
// @author       You
...
(function() {
    ...
    // Show access status message
    function showAccessMessage(title, message, type = 'info') {
        // Remove existing message
        const existing = document.getElementById('haesl-access-status');
        ...
        messageDiv.id = 'haesl-access-status';
    ...
    function checkAccess() {
        return new Promise((resolve) => {
            showAccessMessage('🔍 Access Check', 'Verifying authorization...', 'info');
                ...
                onload: function(response) {
                    if (response.status === 200) {
                        try {
                            ...
                            if (!config.enabled) {
                                showAccessMessage('🚫 System Disabled', 'The CAR extractor is currently disabled.', 'error');
                                resolve(false);
                                return;
                            }

                            if (config.globalSettings?.maintenanceMode) {
                                showAccessMessage('🔧 Maintenance Mode', config.messages?.maintenance || 'System under maintenance', 'error');
                                resolve(false);
                                return;
                            }
                            ...
                            if (!user) {
                                showAccessMessage('🚫 Access Denied', `User ID ${USER_ID} not authorized. Contact administrator.`, 'error');
                                resolve(false);
                                return;
                            }

                            if (!user.enabled) {
                                showAccessMessage('🚫 Access Disabled', 'Your access has been disabled.', 'error');
                                resolve(false);
                                return;
                            }
                            ...
                            if (now >= expiry) {
                                showAccessMessage('⏰ Access Expired', 'Your access has expired.', 'error');
                                resolve(false);
                                return;
                            }

                            // Access granted
                            showAccessMessage('✅ Access Granted', `Welcome ${user.name}! Loading CAR extractor...`, 'success');
                            resolve(true);

                        } catch (e) {
                            showAccessMessage('❌ Config Error', 'Error parsing access control configuration.', 'error');
                            resolve(false);
                        }
                    } else {
                        showAccessMessage('❌ Connection Error', 'Cannot reach GitHub access control.', 'error');
                        resolve(false);
                    }
                },
                onerror: function() {
                    showAccessMessage('❌ Network Error', 'Cannot verify access. Check connection.', 'error');
                    resolve(false);
                }
            });
        });
    ...
    // Main CAR Extractor Script (only loads if access is granted)
    function loadMainScript() {
        ...
        // Include all the main script functions here (abbreviated for space)
        // All the original functions from the working script...

        // Just showing key functions - the full script would include all functions from the original
        ...
        // Placeholder functions - in real implementation, include all functions from original script
        function minimizeRibbon() { /* Implementation from original */ }
        function maximizeRibbon() { /* Implementation from original */ }
        function createContextMenu() { /* Implementation from original */ }
        function startExtraction() { /* Implementation from original */ }
        function pauseExtraction() { /* Implementation from original */ }
        function stopExtraction() { /* Implementation from original */ }
        function exportResults() { /* Implementation from original */ }
        function clearResults() { /* Implementation from original */ }
        function debugPage() { /* Implementation from original */ }
        function updateButtonStates(state) { /* Implementation from original */ }
    ...
    async function initialize() {
        console.log('🔐 HAESL CAR Extractor - Remote Access Control');
        console.log(`👤 User ID: ${USER_ID}`);

        // Check if we're on a details page (popup window for extraction)
        if (window.location.href.includes('#!/details') && window.opener) {
            // Let the original extraction logic handle this
            return;
        }
